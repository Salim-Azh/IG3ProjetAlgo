...
protocol Piece {
    ...
    init(forme : String, couleur : String) //une piece est definie par un sa forme (string) et sa couleur (string)
}


// Structure de donnees du type PIECE
struct Piece : Piece{
    private(set) var forme : String  // declaration de la propriete forme pour un pion
    private(set) var couleur : String  // declaration de la propriete couleur pour un pion
    
    init(forme : String, couleur : String) {  // initialisation d'un pion qui a une forme et une couleur
        self.forme = forme
...
protocol Joueur {
    ...
    init(numero : Int /*,description : String ?????????? */) //Un joueur est definit par un numéro de joueur et un lot de pieces, la fonction init fait donc appel a SetLotDePieces. Le joueur 1 aura automatiquement les blancs et le 2 les rouges.
    ...
    func setLotDePieces () ->  [Piece]  //attribue a un joueur les pieces pour démarrer (deux cylindre, deux carrés, deux spheres et deux pyramides) sous forme de liste. Recupere le numéro du joueur pour savoir quelle couleur attribuer aux pieces
