...
while (fin == false) { // tant qu'il n'y a pas de gagnant
    ...
    if (tour % 2 == 0){  // si tour est pair, c'est au joueur 1 de jouer
        ...
        var piece : Piece = Piece(forme : demandeForme(), couleur : "blanc") // on crée la pièce que le joueur veut jouer.
        
        var positionD : (Int,Int) = demandePosition()
        
        if plateau.place(position : positionD, piece : piece, joueur : Joueur1) == true { //si le joueur a pu placer sa pièce où il voulait
        ...
        var piece : Piece = Piece(forme : demandeForme(), couleur : "rouge")
        
        var positionD : (Int,Int) = demandePosition()
        
        if plateau.place(position : positionD, piece : piece, joueur: Joueur2) == true {
